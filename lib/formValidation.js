...
import { countOccurrences, MAX_INTERVAL, MAX_OCCURRENCES } from './recurring';
...
export function ruleFromForm(form) {
  ...
  // Clamp to the same [1, MAX_INTERVAL] range the generator enforces — the
  // UI stepper already stays inside it, but the rule this produces is
  // persisted and later fed back to the generator, so it must be born valid.
  const interval =
    Number.isInteger(form.repeatInterval) && form.repeatInterval >= 1
      ? Math.min(form.repeatInterval, MAX_INTERVAL)
      : 1;
