...
// updates would be slow and non-atomic. No userId filter here: the RPC is
// SECURITY INVOKER, so RLS scopes it to the caller's own rows.
// `status` is intentionally not sent — each occurrence keeps its own.
// Returns the updated rows (full server rows, mapped through fromDb).
export async function dbUpdateSeriesFrom(seriesId, fromDateISO, dayDelta, base) {
