...
// plus a sub-day fractional component when dueTime is set so same-date
// time-bearing rows sort correctly (9 AM is more urgent than 10 PM).
// No-time rows keep the integer value (backward-compat with existing tests).
// Can be negative (overdue rows). Falls back to DEFAULT_COMPLEXITY when the
// field is absent so pre-migration cached rows sort correctly.
export function adjustedDaysUntilDue(assignment, today = new Date()) {
  ...
  const mins = dueMinutes(assignment);
  const timeFraction = mins !== null ? mins / (24 * 60) : 0;
  return raw - buffer + timeFraction;
}
