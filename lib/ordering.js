...
// out-of-the-box experience reproduces the pre-ranking behavior exactly
// (adjustedDaysUntilDue folds the complexity lead-time buffer into the due
// date score, matching the original hardcoded urgency → importance → date
// ordering bit-for-bit). Any ranking a user actually customizes gets the
// raw (unbuffered) due date instead — otherwise complexity would silently
// influence the dueDate comparison a second time whenever dueDate outranks
// complexity in a custom order, on top of complexity's own factor further
// down the list, contradicting the "independent successive tiebreakers"
// model documented on pickWorkOnNext below.
...
//
// One exception: DEFAULT_RANKING specifically reproduces the pre-ranking
// behavior bit-for-bit, where the due-date score already had the complexity
// lead-time buffer baked in (see FACTOR_COMPARATORS.dueDate's useBuffered).
// Any other ranking gets fully independent factors.
//
